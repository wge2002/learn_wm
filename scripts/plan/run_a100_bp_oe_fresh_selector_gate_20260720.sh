...
# Fresh 60-state locked test for the sparse branch generator and a learned
# outcome-aligned selector.  Candidate-population simulation is disabled;
# only the ordinary K3 final mean and the two retained BP final means are
# executed.

ROOT=/225010117/code/learn_wm
BUNDLE=/225010117/a100_operator_head_20260720
OUT=/225010117/logs/bp_oe_recursive_a100_20260720/fresh_eval_n60_r3_v1
...
EVAL=/225010117/logs/bp_oe_recursive_a100_20260720/fresh_eval_source_h5_off40_n60_seed20260726_v1/source.npz
...
for gpu in 0 1 2 3; do
    ...
    (
        ...
        "${PY}" "${PROBE}" \
            ...
            "+branch_oe.methods='k3_1x300,bp_sparse_matched'" \
...
if ((status != 0)); then
    echo "one or more fresh selector-Gate shards failed" >&2
    exit "${status}"
fi

touch "${OUT}/ALL.done"
echo "fresh sparse selector Gate complete at $(date -Is)"

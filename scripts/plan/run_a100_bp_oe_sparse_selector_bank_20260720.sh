...
# Build recursive sparse-BP branch pairs and final simulator labels on the
# 240-state operator-training bank.  Population execution is disabled: the
# simulator is used only for the two retained final branch means.  These data
# train a separate recursive branch-value selector; evaluation remains on the
# row-disjoint 12-state source.

ROOT=/225010117/code/learn_wm
BUNDLE=/225010117/a100_operator_head_20260720
OUT=/225010117/logs/bp_oe_recursive_a100_20260720/selector_train_n240_r3_v1
...
for gpu in 0 1 2 3; do
    ...
    (
        ...
        "${PY}" "${PROBE}" \
            ...
            +branch_oe.train_source="${SOURCE}" \
            +branch_oe.eval_source="${SOURCE}" \
...
if ((status != 0)); then
    echo "one or more selector-bank shards failed" >&2
    exit "${status}"
fi

touch "${OUT}/ALL.done"
echo "sparse recursive selector bank complete at $(date -Is)"

...
# Cross-horizon counterfactual disagreement Gate.  Unlike the failed shared
# rank ensemble, each model keeps its signed terminal-to-goal vector; the
# population operator uses disagreement to route a branch set instead of
# averaging incompatible scalar rankings.

ROOT=/225010117/code/learn_wm
BUNDLE=/225010117/a100_operator_head_20260720
OUT=/225010117/logs/crossmodel_outcome_set_gate_a100_20260720
...
names=(k3_k5 k3_k10 k3_k5_k10 dense_k3_k5_k10)
...
if ((status != 0)); then
    echo "one or more cross-model outcome probes failed" >&2
    exit "${status}"
fi

touch "${OUT}/ALL.done"
echo "cross-model outcome set Gate complete at $(date -Is)"

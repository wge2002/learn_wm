...
### 机理（这才是有价值的部分）
把专家**特化**到 regime，会让模型对**路由错误脆弱**：错路一个特化专家，比单体泛化器的平均误差**更糟**。**注意机理已被路由探针（`regime_stepB_routing_probe.py`）修正**：我们一度以为"门控在漂移态上变瞎"，实测**推翻**了——监督门控在真状态路由 0.91、在漂移 rollout 态上**仍有 0.887**（floor 0.60），几乎不掉。真正的杀手是**每次错路的灾难性代价**：100% 路由（oracle）mse@10=0.320，~89% 路由（学出门控）就掉到 0.506——~11% 错路 +0.18 drift，碾碎 ~13% 的特化收益。要回本得 ~99% 路由，没有可学门控够得着。软路由不脆弱，但把专家又**混成泛化器**、收益归零（硬=有收益但脆，软=稳但≈单体）。另外，blind 门控在**多步与单步（LeWM 原生 `num_preds=1`）两种训练目标下都照样瞎**（NMI≈0），所以"门控不涌现"**不是多步训练的锅**。
...
- 但它**不转化为可用的控制杠杆**：
  - 当预测器开关（B）：特化让模型对路由错误**脆弱**——门控就算 89% 可靠也不够（要 ~99%），比不特化更差。
...
> ⚠️ **这条"统一洞见"的强度边界**：它主要靠 **Step C**（连 oracle re-ground 都输给均匀，是干净的 oracle 上界否定）撑。**Step B 只排除了"稠密 MLP-MoE 在冻结 latent 上、多步训练"这一种形态**，且整个 B 的 train/eval 与 LeWM 本体不同（见 §4 与 stepB_engineering Part 1）。**没测**的形态仍开放：MoE 进 LeWM 自带 Transformer + 单步训练 + 规划评测、"边界 commit/段内 hold"的稀疏门控、更大容量。所以严格说，B 证的是"这一种开关形态不划算"，不是"regime 当开关原理上不可能"。

这是一组**有机理的负结果**，不是"没调好"——每一步都说清了**为什么失败**以及**需要什么才能成立**（B：要 ~99% 路由才回本，没门控够得着；C：要误差在边界处真的局部化而非累积，但它是累积）。Step D/E 不再单独跑（它们建立在 B/C 的收益上，前提已被否）。

> **最终立场（2026-06-28，结案）**：真正 load-bearing 的是 **oracle 结论**——跨玩具 MLP 与 LeWM 真 Transformer 两个尺度，完美 regime 路由都显著抗漂（~13%，p≈0.03–0.04），即**接触 regime 确实携带真实动力学信息**（干净、假设最少，是 Step A 存在性的功能性确认）。而**"训一个门控去恢复它"本就不是 solid 的做法**：它的失败是引入可学路由器 + ~99% 路由脆弱性墙的**结构性局限**，不是调参问题，也不削弱 oracle 这个事实。⇒ **方向作为"可用方法"收掉；作为"分析结论"(A 存在 + oracle 有值) 保留。** faithful 复核见 [regime_stepA_figures/stepB2_README.md](regime_stepA_figures/stepB2_README.md)。

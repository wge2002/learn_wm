...
> 本 MD = 把三份"已结案的离散尝试"笔记合并成一份，减少 knowledge 目录的散乱。
> 诊断线见 [diagnosis_world_model_drift.md](diagnosis_world_model_drift.md)；**当前定案的活方向**见 [direction_discrete_regime_from_lewm.md](direction_discrete_regime_from_lewm.md)。
> 下面"一览表"是这条线最有用的产物——一张"我们排除了什么、为什么"的参考表。详细原文按节完整保留在后面附录。

## 一览：试过的离散变体 & 怎么死的

| 离散变体 | 放在哪 | 怎么死的 | 出处（见附录） |
| --- | --- | --- | --- |
| 全 latent 量化 / 替换 | 表示 | 量化地板 ≈ drift 量级（C2048→0.084, C128→0.426）；PushT 精细控制要连续精度 | Phase 8 result-8 |
| 事后投影 / snap-back（"拉回锚点"） | 表示，**test-time** | ID drift 是 on-manifold 扩散；投影只降 ~4% mse、**不恢复 planner 排序**（top1/spearman 几乎不变） | Phase 7 B/C |
| 离散承诺子目标（加进 CEM cost） | planner 外挂 | 连续 proposer 恢复 83–90% 碾压离散；min-cost CEM **自带 commit**，承诺项冗余 | Phase 8g/8h/8i；LeWM-twogoal I1/I2/I3 |
| 离散 commitment（toy latent） | selector on 连续 AE latent | latent 控制层**不迁移**（state 空间 +8~+36，latent 消失）；探针证非保真问题 | TwoRoom Exp2/2b；Corridor L0/L0b |
| 离散转移算子（选残差算子，状态连续） | dynamics（冻结 latent，MSE，多步） | `cont U=5` 0.454 打过 best `disc_c` 0.556（差 22–23%）；contraction 只是 regularizer | disc_operator 40k sweep |

## Meta 结论（贯穿全程）

```text
失败的不是"离散"，是"离散被放在 test-time 修正 / planner 外挂"这两个位置：
  - test-time 拉回锚点 → 被 on-manifold 扩散 + 精度地板击穿；
  - planner 外挂承诺 → 被 min-cost CEM 自带 commit 吃掉（"承诺"与"离散"正交）。
唯一未被证伪、且文献正面支持的位置 = "离散作为 training-time 目标"，重塑连续 predictor 的外推。
→ 见 direction_discrete_regime_from_lewm.md。
```

---

# 附录 A：commitment_anchor_discrete.md（原文完整保留）
...
# 附录 B：discrete_operator_new_start.md（原文完整保留）
...
>
> 40k sweep 结果已经记录在 [discrete_operator_40k_sweep_result.md](discrete_operator_40k_sweep_result.md)：当前 `disc_c` 没有打过 `cont U=5`，但 contraction bias 明显优于普通 `disc`。
...
# 附录 C：discrete_operator_40k_sweep_result.md（原文完整保留）

# 离散算子 40k sweep 结果：没有打过连续动力学

记录日期：2026-06-24。

关联背景：[discrete_operator_new_start.md](discrete_operator_new_start.md)。

## 一句话结论

这次 40k PushT latent sweep 没有支持“离散 transition operator 能在 open-loop rollout 上打过连续动力学”的核心假设。

最强结果仍是连续 baseline：

```text
cont, U=5: mean mse@10 = 0.4542
```

最强离散结果是带 spectral norm 的 contractive operator：

```text
disc_c, K=16, U=5: mean mse@10 = 0.5559
disc_c, K=32, U=5: mean mse@10 = 0.5606
```

也就是说，最好的 `disc_c` 仍比 `cont U=5` 差约 22%-23%。
...
```text
outputs/disc_operator/sweep_40k_20260624_0004
```

Git 中保留的 curated 结果：

```text
docs/knowledge/disc_operator_figures/sweep_40k_20260624_0004/
```

关键运行记录：

```text
start        Wed Jun 24 00:04:11 CST 2026
gendata done Wed Jun 24 01:57:02 CST 2026
sweep done   Wed Jun 24 02:07:33 CST 2026
```

latent 数据：

```text
outputs/disc_operator/latent_seq_40k_20260624_0004.npz
size: 344M
```

该 `.npz` 不进 Git，只保留在服务器 raw outputs 中。

sweep 完整性：

```text
total cells: 54
result.json count: 54
all rc=0
```
...
```text
cont   : z_next = z + MLP(history, action)
disc   : z_next = z + sum_c p_c A_c(z) + W_a(action)
disc_c : same as disc, but A_c has spectral norm
```
...
seed   : 0, 1, 2
unroll : 1, 5
K      : 4, 8, 16, 32 for disc/disc_c
epochs : 60
```

## 排名结果

按 final horizon `mse@10` 排名前 10：

| rank | arm | K | U | seed | mse@1 | mse@10 | codes used |
| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | cont | 0 | 5 | 0 | 0.02858 | 0.43549 | - |
| 2 | cont | 0 | 5 | 1 | 0.02893 | 0.43735 | - |
| 3 | cont | 0 | 5 | 2 | 0.02833 | 0.48963 | - |
| 4 | disc_c | 32 | 5 | 0 | 0.06739 | 0.53553 | 21 |
| 5 | disc_c | 16 | 5 | 1 | 0.06929 | 0.54291 | 16 |
| 6 | disc_c | 16 | 5 | 0 | 0.06943 | 0.55847 | 14 |
| 7 | disc_c | 16 | 5 | 2 | 0.07805 | 0.56644 | 16 |
| 8 | disc_c | 32 | 5 | 1 | 0.06728 | 0.57092 | 24 |
| 9 | disc_c | 32 | 5 | 2 | 0.07331 | 0.57526 | 25 |
| 10 | disc_c | 8 | 5 | 1 | 0.08149 | 0.60626 | 8 |

按 arm/K/U 聚合的 `mse@10`：

| arm | K | U | mean mse@10 | std | n |
| --- | ---: | ---: | ---: | ---: | ---: |
| cont | 0 | 5 | 0.45416 | 0.02509 | 3 |
| disc_c | 16 | 5 | 0.55594 | 0.00977 | 3 |
| disc_c | 32 | 5 | 0.56057 | 0.01779 | 3 |
| disc_c | 8 | 5 | 0.61834 | 0.00858 | 3 |
| disc | 8 | 5 | 0.67309 | 0.02962 | 3 |
| disc | 16 | 5 | 0.67888 | 0.07273 | 3 |
| disc | 32 | 5 | 0.68258 | 0.07782 | 3 |
| disc_c | 32 | 1 | 0.68667 | 0.02352 | 3 |
| cont | 0 | 1 | 0.73609 | 0.01890 | 3 |
| disc_c | 4 | 5 | 0.74813 | 0.03527 | 3 |
| disc_c | 16 | 1 | 0.76156 | 0.00522 | 3 |
| disc | 4 | 5 | 0.76354 | 0.01287 | 3 |
| disc | 32 | 1 | 0.80392 | 0.01180 | 3 |
| disc | 16 | 1 | 0.85351 | 0.01988 | 3 |
| disc_c | 8 | 1 | 0.87320 | 0.00539 | 3 |
| disc | 8 | 1 | 0.93214 | 0.01231 | 3 |
| disc_c | 4 | 1 | 0.95346 | 0.00421 | 3 |
| disc | 4 | 1 | 0.98985 | 0.00590 | 3 |

## 读数

### 1. 核心假设没有成立

如果离散算子是抗 diffusion / anti-compounding 的核心机制，`disc_c` 应该在长 horizon 上至少接近或超过 `cont`。

实际结果相反：

```text
cont U=5            mean mse@10 = 0.4542
best disc_c U=5     mean mse@10 = 0.5559
```

这说明当前形式的离散 transition operator 没有成为 open-loop rollout 的优势来源。

### 2. spectral norm 是有用的 regularizer

`disc_c` 明显强于普通 `disc`：

```text
disc_c K=16 U=5: mean mse@10 = 0.5559
disc   K=16 U=5: mean mse@10 = 0.6789
```
...
```text
disc_c K=32 U=5: mean mse@10 = 0.5606
disc   K=32 U=5: mean mse@10 = 0.6826
```

所以 contractive bias 有帮助，但帮助不够大。它更像 regularizer，而不是替代连续动力学的主机制。

### 3. unroll training 比离散/连续更关键

连续 baseline 从 U=1 到 U=5 改善很大：

```text
cont U=1: mean mse@10 = 0.7361
cont U=5: mean mse@10 = 0.4542
```

这说明训练目标是否直接包含 open-loop rollout，比当前这版离散结构更决定结果。

### 4. selector 没有明显 collapse

最好的 `disc_c` 没有退化成只用一个 code：

```text
disc_c K=16 U=5: codes used = 14-16 / 16
disc_c K=32 U=5: codes used = 21-25 / 32
```

因此失败原因不像是 selector collapse，而是算子形式本身没有提供足够优势。

## 关键 caveat

当前 `disc_c` 的 spectral norm 只约束 `A_c`，但实际更新是：

```text
z_next = z + A_c(z) + W_a(action)
```

即使 `||A_c|| <= 1`，整体 `I + A_c` 仍然可能放大误差。

所以当前 `disc_c` 不是严格的 non-expansive transition，只是给残差分支加了 spectral-norm inductive bias。
...
```text
状态连续、离散只选择残差算子的这版，没有打过连续 rollout predictor。
```

但是它不是无信息失败：

- spectral norm / contraction bias 有明显帮助。
- 多步 unroll loss 是必须保留的关键设计。
- K=16/32 比 K=4/8 更合理。
- selector 使用率正常，问题不在 code collapse。

## 下一步建议

如果继续这条线，不建议继续加大当前 `z + A_c(z)` 架构。

更值得测的是严格约束整体转移：

```text
z_next = B_c z + W_a(action)
||B_c|| <= 1
```
...
```text
z_next = z + alpha * A_c(z) + W_a(action)
alpha in {0.05, 0.1, learned bounded scalar}
```

判据仍然保持简单：

```text
如果不能在 mse@10 上接近或超过 cont U=5，就不要进入 planner/control eval。
```

...
### 补测:证书缺口闭合(2026-07-12,probe R² + gauge 距离 + sg drift)

四臂完整档案(新增列 = 本轮补测):

| 臂 | rate | drift₁ | drift₈ | probe agent/block/angle | planning |
| --- | ---: | ---: | ---: | --- | ---: |
| K=1 | 1.29 | 1.05 | 6.12 | .961/.979/.934 | 83.0 |
| TF-K5 | 1.37 | 1.05-1.14 | 6.0 | **.900/.946**/.865 | 69.0 |
| sg-K5 | 1.17 | **1.51/1.56** | 6.5/6.4 | .931/.963/.905 | 54.7 |
| K=5 | 0.98 | 1.60 | 4.7 | .972/.973/.813 | 87.0 |

**两个"单翼"臂的掉分被定量解释,四臂 planning 全部对上账:**
- **sg-K5 = 付了代价没买到货**:drift₁ 1.51(付出 K=5 级的一步精度代价
  1.60)但无驯化(drift₈ 6.5 甚至比 K=1 差)——predictor 为满足多步项
  牺牲一步保真,而 CEM 恰恰用一步复合;
- **TF-K5 = 没付代价但丢了信息**:drift 曲线与 K=1 重合(无动力学代价),
  但 agent/block probe R² 掉到 0.900/0.946(K=1:0.961/0.979)——多偏移
  TF 监督侵蚀 encoder 状态内容;gauge 距离确认 TF 的 gauge 确实偏离了
  K=1(linear R² 0.79 vs K1↔K5 的 0.86)且一无所获;
- **planning ≈ f(状态信息内容, planner-horizon 复合误差)**:
  K=5 付一步代价但买到驯化+信息保持 → 87;K=1 什么都不付不买 → 83;
  TF 丢信息 → 69;sg 付代价无收益 → 55。

**probe R² 的 K 扫描(顺带的不对称故事特征空间面):**
agent_xy 随 K **上升**(.961→.977 @K=10,agent = 动作驱动特征),
angle 随 K 下降(.934→.81-.83,angle = contact 噪声放大特征)
——分配定理(Prop 2)在特征空间的直接显影。

---

## Part VIII. 方法:EchoReg(2026-07-12 发射,进行中)

### 从定理到方法

2×2 证明驯化的充要成分 = 自复合梯度经 encoder 共适应(Part VII);
风险分解证明它压的是不可预测扰动经算子乘积的回声(理论笔记 §6)。
**EchoReg 把这个有效成分显式合成**:

```text
L = L_1step(与 K=1 基线完全同监督位置) + lam*SIGReg + beta*L_echo
L_echo = (1/m) sum_s relu( ||f 滚动(ctx+delta) - f 滚动(ctx)||_s / eps - 1 )^2
```

- delta 只需动作、不需未来监督目标;每步后把扰动**重归一化到差分方向**
  (贪婪功率迭代)——直接压"每步最坏增益",即 III.3 实测的机制本身;
- **单边 relu、目标 = 临界 1**:不奖励塌缩(sg 教训)、不做全局 Lipschitz
  压缩(K=10 教训、与现有 contraction 正则的区别);
- 监督位置零改动、动作通道零触碰(TF 教训 + 不对称定理的构造性使用);
- eps=1.0 ≈ 物理一步误差刻度(drift₁≈1.05)。

### 设计迭代记录(发射前的离线证伪)

各向同性随机 delta 版本**发射前被离线证伪**:192 维随机方向几乎感受不到
σ_max(K=1 模型上中位增益仅 0.58,9% 超 1,罚不触发)——这正是对齐
测量(III.2)的预言:真实误差集中在放大子空间。贪婪重归一化版验证通过:
K=1 模型上爬到 1.08(69% 超 1)、K=5 模型收敛 ~1.0(54%)——罚会咬
K=1 型动力学、放过 K=5 型,目标函数与机制对准。

### 臂与预注册(2026-07-12 发射前锁定)

三臂:`echo_d192_b10`(β=1, seed 3072)、`echo_d192_b10_s1`(β=1, seed 1)、
`echo_d192_b03`(β=0.3, seed 3072);num_steps=8 与 K=5 完全同窗口;
链:`outputs/ratek/run_echo_v1.sh`(planning×3 + 谱 + 对齐 + probe R² +
separation 全自动)。

- **P-echo-1**:rate 驯化到 ~1.0(β=1),且**不深入亚临界**(单边罚在
  临界停手——与 K=10 的 0.82 区分);
- **P-echo-2**:probe R² 保持 K=1 水平(agent ≥.95, block ≥.97)
  ——不付 TF 的信息侵蚀;
- **P-echo-3**:drift₁ ≈ K=1 的 1.05(< K=5 的 1.60)——不付多步拟合的
  一步代价(容量不花在复合上);
- **P-echo-4**:drift₈ ≈ K=5 的 4.7 而非 K=1 的 6.1(驯化生效);
- **P-echo-5(钱在这)**:planning ≥ 87(K=5 水平);若 P-1..4 全中而
  planning 只到 83:驯化+信息+一步精度不是 K=5 优势的全部
  ——共适应还有未识别成分,回炉分析;
- **证伪线**:若 rate 不驯化 ⇒ echo 梯度经 context 到 encoder 的通道
  不足,需要目标侧压力 ⇒ 方法改用混合形式(echo + 轻量 K 步)。

### v1 判决(2026-07-14):灾难性失败,失败模式 = 查表塌缩

(注:07-13 容器重启杀掉训练于最后 epoch 途中;用 epoch-29/30 权重测量,
LR 已退火,偏差可忽略。)

| 臂 | rate | drift₁ | 动作 gain₈ | probe agent/block | planning |
| --- | ---: | ---: | ---: | --- | ---: |
| echo β=1.0 | 0.597 | **7.96** | **0.00** | .918/.953 | **5.3** |
| echo β=1.0 s1 | 0.572 | 8.19 | 0.00 | .919/.956 | 8.7 |
| echo β=0.3 | 0.747 | 6.38 | 0.01 | .909/.953 | 6.7 |

**P-echo-1..5 全灭。**指纹:fit/pred_loss = 0.0077(折合 L2≈1.2)vs
validate = 0.34(L2≈8.1 = 实测 drift₁)——**44 倍泛化裂缝只在 echo 臂**
(对照 TF: .0056/.0031、k2_s1: .0083/.0049,fit≈val)。encoder 完好
(probe 正常、|z| 正常),死的是 predictor。

**机制:固定 ε 的有限差分增益罚教会 predictor 作弊**——最优解是
"查表函数":训练点上精确拟合、ε 球内近乎平坦(增益→0,单边罚白给)、
点间垃圾。动作响应一并压平(gain₈=0),CEM 无信号可排序 → planning 6%。

**耦合论题(三种解耦三种死法,定理级经验模式):**

```text
sg   解耦 encoder   → 付一步代价无驯化收益      → 55 分
TF   解耦复合       → 无驯化 + 信息侵蚀          → 69 分
echo 解耦精度       → 查表塌缩、动作通道死        → 6 分
K=5  全耦合         → 驯化+信息+可控性同时成立    → 87 分
```

K 步目标不可拆的原因:它的增益压力在**自生成 rollout 点上同时要求命中
真目标**——增益与精度在同一批点耦合,没有作弊自由度。
这是论文的一个主论点候选:**多步开环 loss 是这套约束的最小耦合实现;
任何拆解都打开一个作弊通道。**

### v2:γ 剂量混合(2026-07-14 13:06 发射)

保持耦合,把整数旋钮 K 变成连续剂量:

```text
L = L_1step(全权重) + gamma * L_K5_openloop + lam*SIGReg
```

三臂 γ ∈ {0.1, 0.3, 1.0},seed 3072,`run_mix_v2.sh`(GPU 按 07-14 新规:
路径判归属、不看卡号)。理论依据:Prop 2 压力 ∝ 乘积项相对权重,γ 连续
控制 Φ;K=5 原版把 step-1 权重稀释到 1/5(drift₁ 1.60 的来源假说),
混合恢复一步强调。

**预注册(发射前锁定):**
- P-mix-1:rate(γ) 单调下降,在 1.29(γ=0)与 ~0.98(γ→∞≈K=5)之间
  连续插值——剂量-响应曲线本身是 Prop 2 的定量检验;
- P-mix-2:drift₁(γ) 单调上升但 γ 小时 < 1.60(一步代价可调低);
- P-mix-3(钱):存在 γ* 使 planning ≥ 87 且 drift₁ < 1.6
  ——即"K=5 的收益、更低的代价",这就是新方法的主张;

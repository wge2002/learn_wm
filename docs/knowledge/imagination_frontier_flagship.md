...
## 6. Day-0 侦察结果(2026-07-14,零训练,当天完成)

**模型定位更新(PI 指示):旗舰以"新模型"为主体 = CritWM(临界自稳世界模型)
——训练环:echo 探针在线测 rate → 闭环调耦合多步剂量 γ,自稳临界,消掉 K 超参;
规划环:同一探针给 CEM 认证想象前缀(资格门 + 排序校准)。
frontier 协议降为该模型的评测章。PushT = 传感器/执行器标定;OGBench = 整机完整实验。**

### 侦察 1:远 goal 打开边界(4 分 → 16 分)

| 模型 | goal 25 步 | 40 步 | 60 步 |
| --- | ---: | ---: | ---: |
| K=1 D192 | 83.0 | 40.0 | 20.0 |
| K=5 D192 | 87.0 | **56.0** | 24.0 |
| K=10 D192 | 85.7 | 52.0 | 26.0 |
| K=1 D8 | — | 18.0 | 4.0 |
| K=5 D8 | — | 34.0 | 12.0(3×K=1)|

短 goal 评测把驯化收益压成 4 分;40 步处张开到 16 分(两个容量都是)。
"planning 稳健之谜"正面解决:之前 50× 放大差映射不到分数,
是因为评测没走到边界。

### 侦察 2+3:想象深度悬崖(预注册交叉被证伪,信息量更大)

off60,想象深度(planner horizon)与 CEM 搜索预算:

| | h5 s300 | h5 s1000 | h8 s300 | h8 s1000 | h10 s300 |
| --- | ---: | ---: | ---: | ---: | ---: |
| K=1 | 20 | 26 | 14 | 10 | 6 |
| K=5 | 24 | 26 | **4** | **4** | 4 |

- 预注册的"视距交叉"(K=5 从更长想象获益)**被证伪**:更深想象伤害两个模型;
- 3.3× 搜索预算不救 → **不是 CEM 维度伪影,是模型**;
- **驯化模型死得更快**:K=5 超出训练深度 1.6×(h8)即崩,K=1 超出 5×(h5)
  仍工作——可用想象深度不由 rate 或训练 K 单独决定;
- latent 分离度证书未预警(K=5 sep₈=18.5 健康)——**排序死在现有证书
  盲区**,证实 Codex R2 的判断:证书必须在实测排序反转上校准,
  增益/分离度单独都不够。

### 对 CritWM 的设计修正

1. 规划环证书 = 资格门 × 增益 × **排序反转校准**(三件套,前两者已有,
   第三者 = Week-1 首项:CEM 候选级日志 + oracle 排名标签,按深度分层);
2. 训练环新增一条可检验主张:CritWM 的在线剂量控制应当**同时扩展可用
   想象深度**(固定 K=5 的深度悬崖在 h8——若临界自稳训练把悬崖推远,
   这是模型的第二个卖点);
